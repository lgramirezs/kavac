...
<table cellspacing="0" cellpadding="1" border="0">
	...
	@foreach($records as $parent)
		...
		@foreach($parent['children'] as $children2)
			@if($children2['level'] > $level || ($zero === false && $children2['balance'] == 0))
				{{-- No muestra nada --}}
			@else
				<tr>
			...
			@foreach($children2['children'] as $children3)
				@if($children3['level'] > $level || ($zero === false && $children3['balance'] == 0))
					{{-- No muestra nada --}}
				@else
					<tr>
				...
				@foreach($children3['children'] as $children4)
					@if($children4['level'] > $level || ($zero === false && $children4['balance'] == 0))
						{{-- No muestra nada --}}
					@else
						<tr>
					...
					@foreach($children4['children'] as $children5)
						@if($children5['level'] > $level || ($zero === false && $children5['balance'] == 0))
							{{-- No muestra nada --}}
						@else
							<tr>
						...
						@foreach($children5['children'] as $children6)
							@if($children6['level'] > $level || ($zero === false && $children6['balance'] == 0))
								{{-- No muestra nada --}}
							@else
								<tr>
