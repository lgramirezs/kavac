...
export default {
  ...
  watch: {
    perPage(res) {
      if (this.page == 1) {
        this.loadAssets(
          `${window.app_url}/asset/registers/vue-list/${res}/${this.page}`
        );
      } else {
        this.changePage(1);
      }
    ...
    page(res) {
      this.loadAssets(
        `${window.app_url}/asset/registers/vue-list/${this.perPage}/${res}`
      );
    },
  },
  created() {
    this.loadAssets(
      `${window.app_url}/asset/registers/vue-list/${this.perPage}/${this.page}`
    );
    this.getAssetTypes();
